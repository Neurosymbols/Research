...
path = "./app/data/ontologies/epoch4"
input_path = f"./app/data/input/epoch4"
output_path = f"./app/data/output/epoch4"
...
def create_failure_mode_rules():
    ...
    null_specs = [
        name.lower() for name, values in specs.items()
        if all(v is None for v in values.values())
    ]
    #non-null specs
    non_null_specs = [s.lower() for s,v in specs.items() if s.lower() not in null_specs]
    fmea_data_dict = {} 
    rule_count = 0
    for row in target_cols:
        fm = row[1]
...
def generate_synthetic_data(
    ...
    for defect, defect_info in fmrs['failure_causes_rules_mapping'].items():
        for fc_name, fc_data in defect_info.items():
            ...
            if contributing_params:  # not empty
                defect_matrix[fc_data['id']] = defect_matrix[contributing_params].max(axis=1) * fc_data['weight'] * fc_data['severity']
            else:
                # if no contributing params, set it to 0 (or NaN if you prefer)
                defect_matrix[fc_data['id']] = 0
            defect_matrix['rbi_score'] += defect_matrix[fc_data['id']]
        # defect_matrix['rbi_score'] = round(defect_matrix['rbi_score'], 2)
    # get all FC columns
